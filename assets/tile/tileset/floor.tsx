<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.8" tiledversion="1.8.0" name="floor" tilewidth="32" tileheight="32" tilecount="18" columns="6">
 <image source="source/floor.png" width="192" height="96"/>
 <wangsets>
  <wangset name="floor" type="corner" tile="-1">
   <wangcolor name="horizontal" color="#ff0000" tile="-1" probability="1"/>
   <wangcolor name="vertical" color="#00ff00" tile="-1" probability="1"/>
   <wangtile tileid="0" wangid="0,0,0,2,0,0,0,0"/>
   <wangtile tileid="1" wangid="0,0,0,2,0,2,0,0"/>
   <wangtile tileid="2" wangid="0,0,0,0,0,2,0,0"/>
   <wangtile tileid="3" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="4" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="5" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="6" wangid="0,2,0,2,0,0,0,0"/>
   <wangtile tileid="7" wangid="0,2,0,2,0,2,0,2"/>
   <wangtile tileid="8" wangid="0,0,0,0,0,2,0,2"/>
   <wangtile tileid="9" wangid="0,1,0,1,0,0,0,0"/>
   <wangtile tileid="10" wangid="0,1,0,1,0,1,0,1"/>
   <wangtile tileid="11" wangid="0,0,0,0,0,1,0,1"/>
   <wangtile tileid="12" wangid="0,2,0,0,0,0,0,0"/>
   <wangtile tileid="13" wangid="0,2,0,0,0,0,0,2"/>
   <wangtile tileid="14" wangid="0,0,0,0,0,0,0,2"/>
   <wangtile tileid="15" wangid="0,1,0,0,0,0,0,0"/>
   <wangtile tileid="16" wangid="0,1,0,0,0,0,0,1"/>
   <wangtile tileid="17" wangid="0,0,0,0,0,0,0,1"/>
  </wangset>
 </wangsets>
</tileset>
